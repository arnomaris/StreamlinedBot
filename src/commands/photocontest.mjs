...
import { getMessage, setMessage, deleteEntry, getVote } from '../database/photocontestHandler.mjs'
...
export async function execute(interaction) {
    ...
    switch (interaction.options.getSubcommand()) {
        case 'submit':
            ...
            let photoContestOpen = await getSetting('photocontest', interaction.guild.id).catch(err => {
            ...
            if (attachment.contentType.endsWith('png') || attachment.contentType.endsWith('jpeg') || attachment.contentType.endsWith('jpg')) {
                let lastSubmission = await getMessage(interaction.user.id, interaction.guild.id).catch(err => {
                ...
                if (lastSubmission == null) {
                    ...
                    setMessage(interaction.user.id, submission.id, interaction.guild.id).catch(err => {
        ...
        case 'delete':
            await interaction.deferReply({ ephemeral: true })
            var entryId = await getMessage(interaction.user.id, interaction.guild.id).catch(err => {
            ...
            if (entryId) {
                ...
                try {
                    ...
                    let entry = await photocontestChannel.messages.fetch(entryId).catch(err => {
                        deleteEntry(interaction.user.id, interaction.guild.id)
