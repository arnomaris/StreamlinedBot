import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, SlashCommandBuilder, PermissionFlagsBits, SnowflakeUtil } from 'discord.js'
import { getEntry, deleteMessage, getEntries, getVotes } from '../database/photocontestHandler.mjs'
...
export async function execute(interaction) {
    switch (interaction.options.getSubcommand()) {
        ...
        case 'delete':
            ...
            if (message) {
                let userId = await getEntry(messageId, interaction.guild.id)
            ...
            break
        case 'open':
            updateSetting('photocontest', interaction.guild.id, true)
            var photocontestChannel = interaction.guild.channels.cache.find(channel => channel.name === 'photo-contest')
            ...
            break
        case 'close':
            updateSetting('photocontest', interaction.guild.id, false)
            updateSetting('voting', interaction.guild.id, false)
            var photocontestChannel = interaction.guild.channels.cache.find(channel => channel.name === 'photo-contest')
        ...
        case 'openvoting':
            updateSetting('photocontest', interaction.guild.id, false)
            updateSetting('voting', interaction.guild.id, true)
            var photocontestChannel = interaction.guild.channels.cache.find(channel => channel.name === 'photo-contest')
            ...
            let entries = await getEntries(interaction.guild.id)
        ...
        case 'getvotes':
            await interaction.deferReply()
            let votes = await getVotes(interaction.guild.id)
            let embeds = []
            ...
            for (let i in votes) {
                ...
                if (textMessage && member) {
                    currentEmbed.addFields({ name: member.user.username, value: `[Votes: ${entry.votes}](${textMessage.url})` })
                } else {
